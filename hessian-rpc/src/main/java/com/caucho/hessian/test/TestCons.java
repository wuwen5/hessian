...
public class TestCons implements java.io.Serializable {
    ...
    public String toString(HashMap map) {
        ...
        if (_first instanceof TestCons) sb.append(((TestCons) _first).toString(map));
        else sb.append(_first);

        sb.append(",");

        if (_rest instanceof TestCons) sb.append(((TestCons) _rest).toString(map));
        else sb.append(_rest);
